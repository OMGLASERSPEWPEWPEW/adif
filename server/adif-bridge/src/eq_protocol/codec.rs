...
// EQEmu CRC32 lookup table — ported verbatim from common/net/crc32.cpp
#[rustfmt::skip]
pub(crate) const CRC32_TABLE: [u32; 256] = [
...
// Ported line-for-line from EQEmu common/net/crc32.cpp EQ::Crc32(data, size, key)
pub fn crc32(data: &[u8], key: u32) -> u32 {
    let mut crc: u32 = 0xFFFFFFFF;
    for i in 0..4u32 {
        let byte = ((key >> (i * 8)) & 0xFF) as u32;
        crc = ((crc >> 8) & 0x00FFFFFF) ^ CRC32_TABLE[((crc ^ byte) & 0xFF) as usize];
    }
    for &b in data {
...
mod tests {
    ...
    #[test]
    fn crc_matches_eqemu_capture() {
        // From packet capture: world session Ack packet
        // SessionResponse encode_key = 0xFFFFFFFF (from capture bytes 6-9)
        // Ack packet body: 00 15 00 00
        // CRC appended by EQEmu: 9E 64
        let key: u32 = 0xFFFFFFFF;
        let data = vec![0x00u8, 0x15, 0x00, 0x00];
        let crc = crc32(&data, key);
        let crc_be = crc.to_be_bytes();
        // 2-byte CRC = last 2 bytes of big-endian CRC
        assert_eq!(
            [crc_be[2], crc_be[3]],
            [0x9E, 0x64],
            "CRC mismatch: got {:02X} {:02X}, expected 9E 64",
            crc_be[2], crc_be[3]
        );
    }
