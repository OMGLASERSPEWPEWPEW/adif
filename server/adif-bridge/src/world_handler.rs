...
pub async fn handle_world_opcode(
    ...
    match opcode {
        opcodes::OP_SEND_LOGIN_INFO => {
            ...
            // ApproveWorld (544 bytes — hardcoded from EQEmu world/client.cpp:1636)
            #[rustfmt::skip]
            const APPROVE_WORLD: [u8; 544] = [
            ...
            crate::send_app_packet(session, socket, addr, opcodes::OP_APPROVE_WORLD, &APPROVE_WORLD).await?;
